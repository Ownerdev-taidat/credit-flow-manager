...
// PUT /api/members/:id/expiry — set start_date and end_date manually
router.put('/:id/expiry', requireAuth, async (req, res) => {
  const { start_date, end_date } = req.body;
  const member = await db.prepare('SELECT * FROM members WHERE id = ?').get(req.params.id);
  if (!member) return res.status(404).json({ error: 'Member not found' });
  await db.prepare('UPDATE members SET start_date = ?, end_date = ?, updated_at = NOW() WHERE id = ?').run(start_date || null, end_date || null, req.params.id);
