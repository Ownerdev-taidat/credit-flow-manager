...
router.post('/:id/add-member', requireAuthOrBridge, async (req, res) => {
  ...
  try {
    ...
    // Auto-set start_date and end_date for the newly added member
    if (result && result.success) {
      ...
      // Update any member with this email under this admin that doesn't have dates yet
      try {
        await db.prepare("UPDATE members SET start_date = ?, end_date = ? WHERE admin_id = ? AND email = ? AND start_date IS NULL").run(startStr, endStr, parseInt(req.params.id), email);
      } catch (e) { console.error('[AddMember] Auto-set dates error:', e.message); }
    }
    res.json(result);
  } catch (err) {
