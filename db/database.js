...
async function initDatabase() {
  if (USE_PG) {
    ...
    const sqliteMigrations = [
      'ALTER TABLE members ADD COLUMN start_date TEXT',
      'ALTER TABLE members ADD COLUMN end_date TEXT',
    ];
    for (const sql of sqliteMigrations) { try { sqlite.exec(sql); } catch { } }
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      if (USE_PG) {
        ...
        console.log(`[DB] Connecting to: ${dbUrl.replace(/:[^:@]+@/, ':***@')} (attempt ${attempt}/${maxRetries})`);
