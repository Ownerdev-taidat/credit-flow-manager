...
const AdminsPage = {
  ...
  async load() {
    ...
    content.innerHTML = `
      ...
      <div style="margin-bottom:16px;display:flex;gap:8px;align-items:center">
        <div style="position:relative;max-width:320px;flex:1" id="search-wrapper">
          <input type="text" id="farm-search" placeholder="Nhập email tìm kiếm..." style="width:100%;padding:8px 12px 8px 32px;border-radius:8px;border:1px solid rgba(255,255,255,0.12);background:rgba(255,255,255,0.06);color:var(--text-primary);font-size:13px;outline:none" autocomplete="off">
    ...
    // Autocomplete: show dropdown on input
    document.getElementById('farm-search').addEventListener('input', (e) => this.onSearchInput(e.target.value));
    // Hide dropdown when clicking outside
    document.addEventListener('click', (e) => {
      const wrapper = document.getElementById('search-wrapper');
      if (wrapper && !wrapper.contains(e.target)) this.hideSearchDropdown();
    });
    // Show dropdown again on focus if there's text
    document.getElementById('farm-search').addEventListener('focus', (e) => {
      if (e.target.value.trim().length >= 1) this.onSearchInput(e.target.value);
    });
  ...
  async onSearchInput(value) {
    ...
    this._searchDebounce = setTimeout(async () => {
      try {
        const results = await App.api(`/api/dashboard/search?q=${encodeURIComponent(q)}`);
        if (results.length > 0) {
          this.showSearchDropdown(results, q);
        } else {
          this.showSearchDropdown([], q); // Show "no results"
        }
      } catch {
        this.hideSearchDropdown();
      }
  ...
  showSearchDropdown(results, query) {
    ...
    if (results.length === 0) {
      dropdown.innerHTML = `<div class="search-dropdown-empty"><i class="fas fa-search"></i> Không tìm thấy kết quả cho "${query}"</div>`;
    ...
    dropdown.innerHTML = results.map(r => {
      const typeBadge = r.type === 'admin' 
        ? '<span class="search-badge search-badge--admin"><i class="fas fa-user-shield"></i> Admin</span>'
        : '<span class="search-badge search-badge--member"><i class="fas fa-user"></i> Member</span>';
      const emailDisplay = r.email ? highlight(r.email, query) : '<span style="color:var(--text-muted)">N/A</span>';
      const nameDisplay = highlight(r.name || '', query);
      
      return `<div class="search-dropdown-item" onclick="AdminsPage.onSearchSelect(${r.admin_id})" data-admin-id="${r.admin_id}">
        <div class="search-dropdown-avatar" style="background:${r.avatar_color || '#6366f1'}">${(r.name || '?').charAt(0)}</div>
        <div class="search-dropdown-info">
          <div class="search-dropdown-email">${emailDisplay}</div>
          <div class="search-dropdown-meta">${nameDisplay} · <span style="color:var(--accent-light)">${r.admin_name}</span></div>
        </div>
        ${typeBadge}
      </div>`;
    }).join('');
    ...
    try {
      ...
      const membersHtml = members.length ? members.map(m => {
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:rgba(255,255,255,0.03);border:1px solid ${isOverLimit ? 'rgba(239,68,68,0.2)' : 'rgba(255,255,255,0.06)'};border-radius:8px;margin-bottom:5px">
          ...
          <div style="flex:1;min-width:0;overflow:hidden">
            <div style="font-weight:600;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${m.name}${isOverLimit ? ' <span style="color:var(--danger);font-size:10px">⚠ Vượt hạn mức</span>' : ''}${(() => {
              if (!m.end_date) return '';
              const today = new Date().toISOString().split('T')[0];
              const tomorrow = new Date(Date.now() + 86400000).toISOString().split('T')[0];
              const ed = (m.end_date || '').split('T')[0];
              if (ed < today) return ' <span style="background:#ef4444;color:white;padding:1px 6px;border-radius:4px;font-size:9px;font-weight:700">❌ Hết hạn</span>';
              if (ed <= tomorrow) return ' <span style="background:#f59e0b;color:white;padding:1px 6px;border-radius:4px;font-size:9px;font-weight:700">⏰ Sắp hết hạn</span>';
              return '';
            })()}</div>
            <div style="font-size:10px;color:var(--text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${m.email || ''}${m.start_date || m.end_date ? ` · <i class="fas fa-calendar-alt" style="margin:0 2px"></i>${m.start_date ? new Date(m.start_date).toLocaleDateString('vi-VN',{day:'2-digit',month:'2-digit'}) : '?'} → ${m.end_date ? new Date(m.end_date).toLocaleDateString('vi-VN',{day:'2-digit',month:'2-digit'}) : '?'}` : ''}</div>
          </div>
          <div style="display:flex;gap:4px;flex-shrink:0;align-items:center">
            ...
            <button class="btn btn-sm" style="padding:3px 6px;font-size:9px;background:rgba(16,185,129,0.15);color:var(--success);border:1px solid rgba(16,185,129,0.3);cursor:pointer;border-radius:6px;white-space:nowrap" onclick="event.stopPropagation();AdminsPage.setMemberExpiry(${m.id}, '${m.name.replace(/'/g, "\\\\'")}', '${m.start_date || ''}', '${m.end_date || ''}')" title="Set h\u1ea1n s\u1eed d\u1ee5ng">
              <i class="fas fa-calendar-alt"></i> Hạn
            </button>
  ...
  // ========= SET MEMBER EXPIRY ==========
  async setMemberExpiry(memberId, memberName, currentStart, currentEnd) {
    const overlay = document.createElement('div');
    overlay.id = 'expiry-overlay';
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:10000;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease';
    overlay.innerHTML = `
      <div style="background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:16px;padding:24px;width:400px;max-width:90vw;box-shadow:0 20px 60px rgba(0,0,0,0.5);animation:slideUp 0.3s ease">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
          <h3 style="margin:0;font-size:15px;color:var(--text-primary)"><i class="fas fa-calendar-alt" style="color:var(--success);margin-right:6px"></i>Set hạn sử dụng</h3>
          <button onclick="document.getElementById('expiry-overlay').remove()" style="background:none;border:none;color:var(--text-muted);cursor:pointer;font-size:18px;padding:0"><i class="fas fa-times"></i></button>
        </div>
        <div style="background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.2);border-radius:10px;padding:12px;margin-bottom:16px">
          <div style="font-size:13px;font-weight:600;color:var(--text-primary);margin-bottom:4px">${memberName}</div>
          <div style="font-size:11px;color:var(--text-muted)">Hiện tại: <strong style="color:var(--success)">${currentStart ? new Date(currentStart).toLocaleDateString('vi-VN') : 'Chưa set'}</strong> → <strong style="color:${currentEnd ? (new Date(currentEnd) < new Date() ? 'var(--danger)' : 'var(--success)') : 'var(--text-muted)'}">${currentEnd ? new Date(currentEnd).toLocaleDateString('vi-VN') : 'Chưa set'}</strong></div>
        </div>
        <div style="margin-bottom:12px">
          <label style="font-size:12px;color:var(--text-secondary);display:block;margin-bottom:6px"><i class="fas fa-play" style="margin-right:4px"></i>Ngày bắt đầu</label>
          <input type="date" id="expiry-start" value="${currentStart ? currentStart.split('T')[0] : new Date().toISOString().split('T')[0]}" style="width:100%;padding:10px 14px;border-radius:8px;border:1px solid var(--border-color);background:var(--bg-primary);color:var(--text-primary);font-size:14px;outline:none;box-sizing:border-box">
        </div>
        <div style="margin-bottom:12px">
          <label style="font-size:12px;color:var(--text-secondary);display:block;margin-bottom:6px"><i class="fas fa-stop" style="margin-right:4px"></i>Ngày kết thúc</label>
          <input type="date" id="expiry-end" value="${currentEnd ? currentEnd.split('T')[0] : ''}" style="width:100%;padding:10px 14px;border-radius:8px;border:1px solid var(--border-color);background:var(--bg-primary);color:var(--text-primary);font-size:14px;outline:none;box-sizing:border-box">
          <div style="font-size:10px;color:var(--text-muted);margin-top:6px">
            <i class="fas fa-info-circle"></i> Mặc định: 30 ngày từ ngày bắt đầu
          </div>
        </div>
        <div style="display:flex;gap:8px;justify-content:flex-end">
          <button onclick="document.getElementById('expiry-overlay').remove()" class="btn btn-secondary" style="padding:8px 16px;font-size:13px">Hủy</button>
          <button id="expiry-save" class="btn btn-primary" style="padding:8px 16px;font-size:13px;background:var(--success);border-color:var(--success)"><i class="fas fa-check"></i> Xác nhận</button>
        </div>
      </div>
    `;
    document.body.appendChild(overlay);

    // Auto-fill end_date = start + 30 days when start changes
    const startInput = document.getElementById('expiry-start');
    const endInput = document.getElementById('expiry-end');
    if (!currentEnd && startInput.value) {
    ...
    document.getElementById('expiry-save').addEventListener('click', async () => {
      const start = startInput.value;
      const end = endInput.value;
      if (!start || !end) { App.toast('Vui lòng chọn ngày', 'error'); return; }
      if (end < start) { App.toast('Ngày kết thúc phải sau ngày bắt đầu', 'error'); return; }
      try {
        await App.api(`/api/members/${memberId}/expiry`, 'PUT', { start_date: start, end_date: end });
        overlay.remove();
        App.toast(`Đã set hạn: ${new Date(start).toLocaleDateString('vi-VN')} → ${new Date(end).toLocaleDateString('vi-VN')}`, 'success');
        const adminId = this.currentDetailAdminId;
        if (adminId) this.showDetail(adminId);
      } catch (err) { App.toast(err.message, 'error'); }
    });
